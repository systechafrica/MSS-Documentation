module.exports = {
  '/': [
    {
      title: 'FundMaster Glossary',
      children: [
        '/glossary/',
      ],
    },
    {
      title: 'How to Access FundMaster',
      children: [
        '/introduction/',
        '/introduction/landingPage',
        '/introduction/productsSetup',
      ],
    },
    {
      title: 'Admin Panel User Guide',
      children: [
        '/administration/',
        '/administration/usersRegister',
        '/administration/profilesRegister',
        '/administration/profilePermissions',
        '/administration/passwordPolicySettings',
        '/administration/notifications',
        '/administration/sms',
        '/administration/email',
        '/administration/auditTrail',
        '/administration/systemRights',
        '/administration/systemUtility',
        '/administration/otherAdminSettings',
      ],
    },
    {
      title: 'Schemes Setup User Guide',
      children: [
        '/setupscheme/newScheme',
        '/setupscheme/basicSchemeDetails',
        '/setupscheme/configurations',
        '/setupscheme/incomeDrawdown',
        '/setupscheme/moduleAllowance',
        '/setupscheme/districts',
        '/setupscheme/villages',
        '/setupscheme/regions',
        '/setupscheme/trustees',
        '/setupscheme/serviceProviders',
        '/setupscheme/sponsors',
        '/setupscheme/costCenters',
        '/setupscheme/principalOfficers',
        '/setupscheme/memberClasses',
        '/setupscheme/vestingScales',
        '/setupscheme/schemeApprovalProcess',
        '/setupscheme/otherSetup',
        '/setupscheme/actuarialFactors',
        '/setupscheme/statutory',
        '/setupscheme/memberClassSpecificInfo',
        '/setupscheme/riskAssessment',
        '/setupscheme/bankers',
      ],
    },
    {
      title: 'Accounts User Guide',
      children: [
        '/accounts/',
        '/accounts/budget',
        '/accounts/cashManagement',
        '/accounts/creditorsDebtors',
        '/accounts/fixedAssets',
        '/accounts/generalLedger',
        '/accounts/intro',
        '/accounts/journals',
        '/accounts/payablesPayments',
        '/accounts/receivableReceipts',
        '/accounts/setup',
      ],
    },
    {
      title: 'Pensioners User Guide',
      children: [
        '/pensioners/',
        '/pensioners/approval',
        '/pensioners/deductionAndArrears',
        '/pensioners/introduction',
        '/pensioners/liabilities',
        '/pensioners/onboarding',
        '/pensioners/payroll',
      ],
    },
    {
      title: 'Reports Engine User Guide',
      children: [
        '/reports/',
        '/reports/batchMembersReport',
        '/reports/dashboardComponents',
        '/reports/gettingStarted',
        '/reports/Introduction',
        '/reports/membersBenefitReport',
      ],
    },
    {
      title: 'Workflow User Guide',
      children: [
        '/workflow/',
        '/workflow/adminPanel',
        '/workflow/analysis',
        '/workflow/intro',
        '/workflow/taskMgt',
      ],
    },
    {
      title: 'Investments User Guide',
      children: [
        '/investments/',
        '/investments/bond&Securities',
        '/investments/deposits',
        '/investments/equity',
        '/investments/fundManagement',
        '/investments/intro',
        '/investments/posting',
        '/investments/privateEquities',
        '/investments/property',
        '/investments/treasuryBills',
      ],
    },
    {
      title: 'Videos Tutorials',
      children: [
        '/videotutorials/',
        '/videotutorials/employerEnrollment',
        '/videotutorials/membersModule',
        '/videotutorials/schemeSetup',
      ],
    },
    {
      title: 'Installation Guide',
      children: [
        '/deployment/',
        '/deployment/postgres',
      ],
    }

  ],
};

